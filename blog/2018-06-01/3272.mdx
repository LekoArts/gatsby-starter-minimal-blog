...
these token can be restricted as one time usable only or by an expiration date.</blockquote>
...
<blockquote>Fractal got the beginning of a revamped directory (from one of our GSoC interns), as well as misc bugfixes and performance improvements. It has also been added to <a href="https://l10n.gnome.org/">Damned lies</a>, the GNOME platform for translations.</blockquote>
...
<blockquote>The user can now change the following settings: Avatar, display name, add and remove email addresses and phone numbers. Also, they can see the homeserver and their own MXID.
We will probably make some more small changes on the way to make the UX as good as we possibly can.</blockquote>
